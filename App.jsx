...
export default function App() {
  ...
  useEffect(() => {
    ...
    // Lógica para funcionar tanto no ambiente de teste (Canvas) quanto na Vercel
    let finalConfig = vercelFirebaseConfig;
    if (typeof __firebase_config !== 'undefined') {
      finalConfig = JSON.parse(__firebase_config);
    }

    // Só inicia se houver configuração
  ...
  // 1. Puxar os dados da Nuvem (Baixar)
  const loadDataFromCloud = async (user) => {
    ...
    try {
      ...
      const docPath = typeof __app_id !== 'undefined' 
        ...
        : doc(dbRef.current, 'users', user.uid, 'plannerData', 'main'); // Caminho padrão Vercel
      
      const snapshot = await getDoc(docPath);
      
      if (snapshot.exists()) {
        const data = snapshot.data();
        // Atualiza os estados locais com os dados da nuvem
  ...
  // 2. Enviar os dados para a Nuvem (Fazer Backup)
  // Utilizamos um debounce (atraso) para não enviar dados a cada letra digitada
  useEffect(() => {
    ...
    // Atrasa o salvamento em 2 segundos após a última alteração
  ...
  const renderRoutine = () => {
    ...
    // Proteções adicionadas para evitar tela branca se os dados vierem nulos da nuvem
